    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans print:bg-white print:text-black">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 md:py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 print:p-0">
        ...
        <section className="lg:col-span-7 flex flex-col items-center justify-start print-full-width">
          ...
          <div className="w-full max-w-[800px] bg-white text-slate-900 shadow-2xl rounded-2xl overflow-hidden border border-slate-200 flex flex-col justify-between print-full-width print:border-none print:shadow-none print:rounded-none min-h-[1120px] p-8 md:p-12 relative">
            
            {/* Elegant luxury top border strip */}
            <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-slate-900 via-amber-500 to-slate-900" />
            
            {/* DOCUMENT BODY FOR PRINT CONTENT */}
            <div className="space-y-8">
              
              {/* BRAND HEADER */}
              <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b pb-6 premium-gold-border">
                {/* Logo Section */}
                <div className="flex items-center gap-4">
                ...
                <div className="text-right md:max-w-[60%] space-y-1">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-900 text-white premium-header-bg border-b-2 border-amber-500">
                      <th className="py-3.5 px-4 text-xs font-bold uppercase tracking-widest rounded-tl-lg text-center w-12">No.</th>
                      <th className="py-3.5 px-4 text-xs font-bold uppercase tracking-widest">Machinery / Equipment Specification</th>
                      <th className="py-3.5 px-4 text-xs font-bold uppercase tracking-widest text-center w-16">Qty</th>
                      <th className="py-3.5 px-4 text-xs font-bold uppercase tracking-widest text-right w-36">Unit Price</th>
                      <th className="py-3.5 px-4 text-xs font-bold uppercase tracking-widest text-right rounded-tr-lg w-36">Total ({currency})</th>
                    </tr>
                  </thead>
                    ...
                    {items.map((item, index) => {
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="py-4 px-4 text-xs text-slate-900 font-semibold leading-relaxed">
                            {item.description}
                          </td>
                          <td className="py-4 px-4 text-xs text-slate-900 font-bold text-center">{item.qty}</td>
                          <td className="py-4 px-4 text-xs text-slate-800 text-right font-medium">
                            {item.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            <span className="text-[10px] text-slate-500 block">per {item.unit}</span>
                          </td>
                          <td className="py-4 px-4 text-xs text-slate-900 font-bold text-right">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start pt-4">
                <div className="md:col-span-7 space-y-2">
                  ...
                  <p className="text-xs italic text-slate-700 bg-slate-50 p-3.5 rounded-lg border border-slate-100 font-medium">
                    {numberToWords(calculateTotal())}
                  </p>
                </div>
                
                <div className="md:col-span-5 bg-slate-50 p-4 rounded-xl space-y-2.5 border border-slate-100">
                  ...
                  <div className="flex justify-between items-center text-xs text-slate-600 pb-2 border-b">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-900">Total Proposal Value:</span>
            ...
            <div className="pt-12 space-y-6">
              ...
              {/* Signatures Layout */}
              <div className="grid grid-cols-2 gap-8 text-center pt-6 border-t border-slate-100">
